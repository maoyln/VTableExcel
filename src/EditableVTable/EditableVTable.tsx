...
const EditableVTable: React.FC<EditableVTableProps> = ({
  ...
  useEffect(() => {
    ...
    const vtableColumns = columns.map((col) => ({
      ...
      customRender: (args: { value: string; col: number; row: number }) => {
        if (typeof args.value === "string" && args.value.startsWith("=")) {
          const isHover =
            hoverCellRef.current.col === args.col &&
            hoverCellRef.current.row === args.row;
          let displayText: string | number = args.value;
          let fill = "gray";
          if (!isHover) {
            try {
              displayText =
                calculateCellFormulaOptimized(
    ...
    table.on(
      "cell_mouseenter" as any,
      ({ col, row }: { col: number; row: number }) => {
        hoverCellRef.current = { col, row };
        setRerender((v) => v + 1);
      }
    );
    table.on("cell_mouseleave" as any, () => {
