...
class express {
  ...
  // 比较运算符优先级
  comparePriority (opa: { value: string }, opb: { value: string }) {
  ...
  // 新增操作数
  addOperand (operand: any, operandStack: any[]) {
    operandStack.push({
      type: operand.type,
      value: operand.value,
  ...
  // 新增运算符
  addOperator (operator: any, operatorStack: any[]) {
    operatorStack.push({
      type: operator.type,
  ...
  // 处理括号
  handleParenthesis (operatorStack: any[], operandStack: any[], qcount: { length: number }) {
    let bool = true
    while (bool) {
  ...
  // 处理操作符
  handleOperator (curOperator: any, curToken: any, operatorStack: any[], operandStack: any[]) {
    curOperator = curToken
  ...
  // 处理标识符
  handleIdentifier (curOperand: any, curToken: any, operandStack: any[]) {
