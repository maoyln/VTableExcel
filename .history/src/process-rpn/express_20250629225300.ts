...
// 获取运算符优先级，数值越大，优先级越低
function getPriority (op: { value: string }) {
...
class express {
  ...
  comparePriority (opa: { value: string }, opb: { value: string }): number {
    const a = getPriority(opa as { value: string })
    const b = getPriority(opb as { value: string })
    if (a > b) {
      return -1
    } else if (a === b) {
      return 0
  ...
  // 新增操作数
  addOperand (operand: Token, operandStack: Token[]) {
    operandStack.push({
      type: operand.type,
      value: operand.value,
  ...
  // 新增运算符
  addOperator (operator: Token, operatorStack: Token[]) {
    operatorStack.push({
      type: operator.type,
  ...
  createToken (type: string, value: any) {
    const token: Token = {
      type,
      value
    }
  ...
  // 处理括号
  handleParenthesis (operatorStack: Token[], operandStack: Token[], qcount: { length: number }) {
    let bool = true
    while (bool) {
  ...
  // 处理操作符
  handleOperator (curOperator: Token | null, curToken: Token, operatorStack: Token[], operandStack: Token[]) {
    curOperator = curToken
  ...
  // 处理标识符
  handleIdentifier (curOperand: Token | null, curToken: Token, operandStack: Token[]) {
