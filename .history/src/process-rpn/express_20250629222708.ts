...
interface Token {
  type: string
  value: string | number | boolean | Token[][] | null
  callee?: string[]
  args?: Token[][]
}
...
class express {
  ...
  comparePriority (opa: Token, opb: Token): number {
    const a = getPriority({ value: typeof opa.value === 'string' ? opa.value : String(opa.value) });
    const b = getPriority({ value: typeof opb.value === 'string' ? opb.value : String(opb.value) });
    if (a > b) {
      return -1;
    } else if (a === b) {
      return 0;
  ...
  addOperator (operator: Token, operatorStack: Token[]): void {
    operatorStack.push({
      type: operator.type,
      value: typeof operator.value === 'string' ? operator.value : String(operator.value)
    })
  }
  ...
  createToken (type: string, value: string | number | boolean | Token[][] | null): Token {
    const token: Token = {
      type,
      value
    }
  ...
  handleParenthesis (operatorStack: Token[], operandStack: Token[], qcount: { length: number }): void {
    ...
    while (bool) {
      ...
      if (typeof last.value === 'string' && last.value === '(') {
        operatorStack.pop()
        qcount.length--
        bool = false
        // break
  ...
  handleOperator (curOperator: Token | null, curToken: Token, operatorStack: Token[], operandStack: Token[]): void {
    ...
    if (operatorStack.length === 0) {
      ...
      // 对比操作符优先级
      if (
        this.comparePriority(curOperator, last) === 1
      ) {
        this.addOperator(curOperator, operatorStack)
      } else {
        ...
        while (operatorStack.length > 0) {
          ...
          if (
            this.comparePriority(curOperator, top) < 1 &&
            typeof top.value === 'string' && top.value !== '('
          ) {
            this.addOperand(operatorStack.pop() as Token, operandStack)
            if (operatorStack.length === 0) {
              this.addOperator(curOperator, operatorStack)
  ...
  handleIdentifier (curOperand: Token | null, curToken: Token, operandStack: Token[]): void {
    ...
    curOperand.callee.push(curToken.value as string)
    // 循环处理属性调用
    while (true) {
      // 记录当前字符位置
      ...
      if (nextToken.type === TOKEN_IDENTIFIER) {
        curOperand.callee.push(nextToken.value as string)
  ...
  // 是否是布尔值
  isBoolean (value: string | number | boolean | Token[][] | null): boolean {
  ...
  matchIdentifier (c: string): Token {
    ...
    while (this.charIndex < this.expression.length) {
      ...
      if (typeof c === 'string' && isAlphaOrLineOrintegerber(c)) {
        this.charIndex++
        name = name + c
      } else if (c === '(') { // 函数
  ...
  escapedString (str: string): string {
    ...
    try {
      // 保证 _str 为 string 类型
      return JSON.parse(`{"string":"${String(_str)}"}`).string
    } catch (e) {
      return _str
    }
  }
