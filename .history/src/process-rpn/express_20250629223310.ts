...
function isOperator (c) { return /[+\-x÷*/^%=()><!:]/.test(c) }
function isOperationalCharacter (c) { return /[+\-x÷*/^%=(]/.test(c) }
function isDigit (c) { return /[0-9]/.test(c) }
function isAlphaOrLine (c) { return /[a-zA-Z_$]/.test(c) }
function isAlphaOrLineOrintegerber (c) { return /[0-9a-zA-Z_${}]/.test(c) }
function isWhiteSpace (c) { return /\s/.test(c) }
function isString (c) { return /[^"']/.test(c) }
function isQuotes (c) { return /"|'/.test(c) }
function isComma (c) { return /,/.test(c) }
...
class express {
  ...
  // 新增操作数
  addOperand (operand, operandStack) {
    operandStack.push({
      type: operand.type,
      value: operand.value,
  ...
  // 新增运算符
  addOperator (operator, operatorStack) {
    operatorStack.push({
      type: operator.type,
  ...
  _expr () {
    ...
    while (operatorStack.length > 0) {
      this.addOperand(operatorStack.pop(), operandStack)
    }
    const finalTokens = []
  ...
  // 处理括号
  handleParenthesis (operatorStack, operandStack, qcount) {
    let bool = true
    while (bool) {
  ...
  // 处理操作符
  handleOperator (curOperator, curToken, operatorStack, operandStack) {
    curOperator = curToken
  ...
  // 处理标识符
  handleIdentifier (curOperand, curToken, operandStack) {
