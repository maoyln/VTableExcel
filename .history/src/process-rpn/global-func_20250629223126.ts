...
export const sum: [string, Func] = ['SUM', function (this: GlobalFuncThis, ...args: (number | string | (number | string | unknown[])[])[]): number {
  ...
  for (const arg of args) {
    if (Array.isArray(arg)) {
      // 如果是数组，递归处理
      total += (this.sum as (...args: (number | string | (number | string | unknown[])[])[]) => number)(...(arg as (number | string | (number | string | unknown[])[])[]));
...
export const count: [string, Func] = ['COUNT', function (this: GlobalFuncThis, ...args: (number | string | (number | string | unknown[])[])[]): number {
  ...
  for (const arg of args) {
    if (Array.isArray(arg)) {
      count += (this.count as (...args: (number | string | (number | string | unknown[])[])[]) => number)(...(arg as (number | string | (number | string | unknown[])[])[]));
